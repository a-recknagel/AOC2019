...
def amplifier_loop(program, phases):
    ...
    amplifiers = [IntCode(program, [phases[0], 0])]
    ...
    while True:
        try:
            ...
            if current == (len(amplifiers) - 1):
                return amplifiers[current].output[0]
            current = (current + 1) % len(amplifiers)
...
def part_2(data):
    return max(amplifier_loop(data, phase) for phase in permutations([5, 6, 7, 8, 9]))
